    ...
    render: (h, { props, children }) => {
        ...
        return (
            <div class="modal-header">
                {context && context.dismissible && (
                    <button type="button" class="close" on-click={emitClose} aria-label="Close">
...
export default {
    ...
    created() {
        ...
        this.$on('show', () => {
            if (!this.$isServer) document.body.classList.add('modal-open')

            this.visible = true
        })
        this.$on('hide', () => {
            if (!this.$isServer) document.body.classList.remove('modal-open')

            this.visible = false
        })
        this.$on('toggle', () => {
            if (!this.$isServer) document.body.classList.toggle('modal-open')
    ...
    methods: {
        ...
        _injectModal() {
            ...
            const injectDiv = () => document.body.appendChild(document.createElement('div'))
        ...
        _renderModal() {
            return (
                <div class="modal in" on-click={emitEvent('hide', this)} style="display: block;">
                    <div
                        on-click={ev => ev.stopImmediatePropagation()}
                        class="modal-dialog"
                        role="document"
                    >
                        <div class="modal-content">
                            {this.$slots.header && (
                                <ModalHeader context={this}>{this.$slots.header}</ModalHeader>
                            )}
